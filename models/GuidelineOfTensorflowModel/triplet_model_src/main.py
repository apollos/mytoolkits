...
# data reading args
flags.DEFINE_string("train_data", "/Users/kaiwu/Documents/POC/TripletLoss/face/train/", "training images' path")
flags.DEFINE_integer("num_input_threads", 4, "number of parallel input threads")

flags.DEFINE_string("test_data", '/Users/kaiwu/Documents/POC/TripletLoss/face/train/', "test images' path")
...
flags.DEFINE_integer("images_per_class", 4, "Number of images per class.")
flags.DEFINE_integer("batch_size", 32, "Number of images per batch")

# network args
# Choose your image preprocessing
flags.DEFINE_string('preprocessing_type', 'inception_v3', 'image processing type')
...
flags.DEFINE_string("weights", None, "weight file directory")
...
def train():
    ...
    with tf.device(tf.train.replica_device_setter(
        ...
        _, end_points = embedding_network(data_placeholder)
        embeddings = end_points[FLAGS.end_point_name]
    ...
    for var in tf.global_variables()+tf.local_variables():
        if var.name in var2restore:
            var_list.append(var)
        if var.name == 'InceptionV3/Conv2d_1a_3x3/BatchNorm/moving_mean:0':
            tf.add_to_collection('monitored variables', var)
