...
def get_default_customer_words_file():
    d = os.path.dirname(os.path.realpath(__file__))
    return os.path.join(d, 'userdict.txt')
...
class WordSegmentation(object):
    ...
    def __init__(self, stop_words_file = None, allow_speech_tags = util.allow_speech_tags):
        ...
        self.user_customer_file = get_default_customer_words_file()
        self.customer_words = set()
        if type(stop_words_file) is str:
            self.stop_words_file = stop_words_file
        for word in codecs.open(self.stop_words_file, 'r', 'utf-8', 'ignore'):
            self.stop_words.add(word.strip())
        jieba.load_userdict(self.user_customer_file)
    ...
    def segment(self, text, lower = True, use_stop_words = True, use_speech_tags_filter = False):
        ...
        # 去除特殊符号
        word_list = [w.word.strip() for w in jieba_result if w.flag!='x' or len(w.word.strip()) >= 2]
